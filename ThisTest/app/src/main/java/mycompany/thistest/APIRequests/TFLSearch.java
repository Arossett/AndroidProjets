...
import mycompany.thistest.TFL.BikePointsList;
...
public class TFLSearch {
    ...
    private String BIKE_POINT = "http://api.tfl.gov.uk/BikePoint/";
    ...
    public BikePoint searchBikePoint(String id)
            throws Exception {
        ...
        try {
            ...
            HttpRequest request = httpRequestFactory
                    .buildGetRequest(new GenericUrl(BIKE_POINT));
            BIKE_POINT += BIKE_POINT+ URLEncoder.encode("{ids}?", "UTF-8");
            request.getUrl().put("ids", id);

            BikePoint bikePoint = request.execute().parseAs(BikePoint.class);
            //ArrayList<Spot> spots = new ArrayList<Spot>(list.places);
